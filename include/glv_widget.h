...
public:
	...
	/// @param[in] r		geometry
	/// @param[in] data		data to initialize from
...
protected:
	...
	float dx(int id=0) const { return w/model().size(id); } // width, in pixels, per element
	float dy(int id=1) const { return h/model().size(id); } // height, in pixels, per element
