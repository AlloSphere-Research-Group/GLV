...
public:
	...
	const Tv& values() const { return mVal; }		///< Get non-mutable reference to values
...
protected:
	...
	std::map<int, Tm *> mVariables;	// model variables to sync to, index-pointer
	//std::map<int, Var> mVariables;
	Tm mMin, mMax;
